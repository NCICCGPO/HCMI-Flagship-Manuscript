...
def build_result(symbol, columns, cancer_gene_census):
    CancerGeneCensus_list = []
    for key in columns:
        result = get_record(symbol, key, cancer_gene_census)
        if key == 'Hallmark':
            if result == 'Yes':
                result = 'https://cancer.sanger.ac.uk/cosmic/census-page/' + symbol
            else:
                result = ''
    ...
    return ('|').join(CancerGeneCensus_list)
...
#    grab the gene symbol from the first annotation ALT
    columns = ['Tier', 'Hallmark', 'Somatic', 'Germline',
    ...
    symbol = csq_dicts[0]['SYMBOL']
    if symbol in cancer_gene_census['Gene Symbol'].tolist():
        CancerGeneCensus = build_result(symbol, columns, cancer_gene_census)
    else:
        CancerGeneCensus = ('|').join(['' for i in columns])
    CancerGeneCensus_line = ','.join([CancerGeneCensus] * len(record.alts))
