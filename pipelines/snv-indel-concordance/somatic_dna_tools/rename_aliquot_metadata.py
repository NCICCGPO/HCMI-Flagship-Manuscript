...
class Variant(object):
    ...
    def __init__(self, line, prefix='', description_prefix=''):
        self.line = line
        self.prefix = prefix
    ...
    def get_info(self):
        ...
        for item in self.info:
            if '=' in item:
                info_dict.update({self.prefix + item.split('=')[0] : '='.join(item.split('=')[1:])})
            elif item != '.':
    ...
    def write(self):
        if len(self.info_dict) > 0:
            info_string = ';'.join(['='.join([x for x in [key, self.info_dict[key]] if x != None])
                                    for key in self.info_dict])
        else:
            info_string = self.info[0] # for '.'
        line = [self.chrom,
                self.pos,
                self.id,
                self.ref,
...
def fix_header(line, prefix, description_prefix):
    '''
        ...
        wording.
    '''
    full_description_prefix = prefix[0:-1] + '  ' + description_prefix + ' : '
    if '##FORMAT=' in line or \
            '##INFO=' in line:
        id = re.search('ID=(?P<id>[^>,]+)', line)
        description = re.search('Description="(?P<description>[^>,]+)', line)
        if id == None:
        ...
        if id.group(1) != 'GT':
            ...
            line = line.replace('Description="' + description.group(1),
                                'Description="' + full_description_prefix + description.group(1))
    return line


def load_header(vcf_in, prefix, description_prefix):
    '''
        Load a VCF file header as a list of lines.
    '''
    with open(vcf_in) as vcf:
        header = [fix_header(line, prefix, description_prefix) for line in vcf if line.startswith('#')]
    return header


def load_vcf(vcf_in, prefix, description_prefix):
    '''
    ...
    with open(vcf_in) as vcf:
        for line in vcf:
            if not line.startswith('#'):
                yield Variant(line, prefix, description_prefix).write()




def rename_metadata(vcf_file, vcf_out_file, 
    ...
    #  rename
    #  =====================
    header = load_header(vcf_file, prefix, description_prefix)
    vcf_reader = load_vcf(vcf_file, prefix, description_prefix)
    #  =====================
    #  rewrite
    #  =====================
...
def main():
    ...
    prefix = sys.argv[3] + '_'
    description_prefix = sys.argv[4]
    assert os.path.isfile(vcf_file), 'Failed to find prep caller VCF call file :' + vcf_file
    rename_metadata(vcf_file, vcf_out_file, prefix, description_prefix)
