...
class Variant():
    ...
    def __init__(self, record, samples, tumor):
        self.record = record
    ...
    def replace_empty(self, value, key, sep=','):
        ...
        if key  == 'GT':
            return '/'.join(['.' if x == None else str(x) for x in value])
        if (isinstance(value, collections.abc.Iterable) and not isinstance(value, str)) or \
    ...
    def find_tools(self):
        ...
        for sample_name in self.samples:
            ...
            if len(found_keys) > 0:
                self.final_samples.append(sample_name)
    ...
    def write(self):
        ...
        line = [self.chrom,
                ...
                self.info,
                self.format]
        line += [self.reduce_samples(self.final_samples)]
...
def vcf_writer(bcf_in, vcf_out_file, tumor):
    '''
    ...
    with open(vcf_out_file, 'w') as vcf_out:
        ...
        for record in bcf_in.fetch():
            out = Variant(record, samples, tumor)
...
def main():
    ...
    vcf_writer(bcf_in, vcf_out_file, tumor)
