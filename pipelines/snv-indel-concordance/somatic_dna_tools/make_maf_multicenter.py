...
def get_HGVSp_Short(HGVSp_string, HGVSc_string, csq_term):
    ...
    if csq_term == 'splice_acceptor_variant' \
        ...
        if len(HGVSc_string.split(':'))>1:
            ...
            HGVSc_coding = re.findall(r'^c.(\d+)',HGVSc_string)
...
def get_sample_indices(tumor, normal, bcf_in):
    print(bcf_in.header.samples[0], bcf_in.header.samples[1])
    print('known:', tumor, normal)
...
def make_row(record, csq_columns, bcf_in, library,
    ...
    for i, alt in enumerate(record.alts):
        hugo = 'Unknown'
        entrez_id = ''
        if csq_dicts[i]['SYMBOL_SOURCE'] == 'EntrezGene':
            entrez_id = csq_dicts[i]['Gene']
            hugo = csq_dicts[i]['SYMBOL']
        # multicenter calls
        center = ';'.join(record.info['centers_called_by'])
        called_by = ';'.join(record.info['called_by'])
        MultiCallerCalled = record.info['MultiCallerCalled']
        ...
        BAM_file= ''
        Sequencer = 'Illumina'
        if 'AD' in record.samples[tumor].keys() \
                and 'AD' in record.samples[normal].keys():
            t_alt_count = record.samples[tumor]['AD'][1]
            t_ref_count = record.samples[tumor]['AD'][0]
            n_alt_count = record.samples[normal]['AD'][1]
            n_ref_count = record.samples[normal]['AD'][0]
        else:
            t_alt_count = ''
            t_ref_count = ''
            n_alt_count = ''
        ...
        for csq_term in csq_dicts[i]['Consequence'].split('&'):
            ...
            line = [hugo, entrez_id, center, ncbi_build, chrom, start, end, strand,
                    ...
                    n_alt_count, n_ref_count, HGVSc, HGVSp, HGVSp_Short, SYMBOL, SYMBOL_SOURCE, IMPACT, MultiCallerCalled, called_by]
...
def write_file(bcf_in, out, csq_columns, library,
               normal, tumor, VEP_version):
    '''
        Write out the header
    '''
    mg = mygene.MyGeneInfo()
    with open(out, 'w') as o:
        header = ['Hugo_Symbol', 'Entrez_Gene_Id', 'Center', 'NCBI_Build', 'Chromosome', 'Start_Position', 'End_Position', 'Strand', 'Variant_Classification', 'Variant_Type', 'Reference_Allele', 'Tumor_Seq_Allele1', 'Tumor_Seq_Allele2', 'dbSNP_RS', 'dbSNP_Val_Status', 'Tumor_Sample_Barcode', 'Matched_Norm_Sample_Barcode', 'Match_Norm_Seq_Allele1', 'Match_Norm_Seq_Allele2', 'Tumor_Validation_Allele1', 'Tumor_Validation_Allele2', 'Match_Norm_Validation_Allele1', 'Match_Norm_Validation_Allele2', 'Verification_Status', 'Validation_Status', 'Mutation_Status', 'Sequencing_Phase', 'Sequence_Source', 'Validation_Method', 'Score', 'BAM_file', 'Sequencer', 't_alt_count', 't_ref_count', 'n_alt_count', 'n_ref_count', 'HGVSc', 'HGVSp', 'HGVSp_Short', 'SYMBOL', 'SYMBOL_SOURCE','IMPACT', 'MultiCallerCalled', 'VariantCallers']
        o.write('\t'.join(header) + '\n')
        normal_index, tumor_index = get_sample_indices(tumor, normal, bcf_in)
...
def main():
    ...
    parser.add_argument('-u', '--intersection-maf',
                     dest='intersection_maf',
                     help='MAF output file')
    parser.add_argument('-l', '--library',
                     dest='library',
                     help='Sequence library type',
                     choices=['WGS', 'Exome', 'WgsExome'])
    ...
    maf[maf.MultiCallerCalled == True].to_csv(args.intersection_maf,
                                                sep='\t', index=False)
