...
def __main__():
    ...
    parser.add_argument('--called-by', help='Variable to indicate tools that called a variant', type=str,default='called_by')
...
#        keep_mnv=1
        ...
        for snvID in MNV[varID]["SNVs"]:
            if args.called_by in SNV[snvID]:
                called_by.append(SNV[snvID][args.called_by])
        ...
        if len(set(called_by)) > 1 \
            ...
            if args.called_by in MNV[varID]:
                snv_callers = called_by[0].split(",")
                mnv_callers=MNV[varID][args.called_by].split(",") # check who called the MNV
...
#    header["FILTER"].append('##FILTER=<ID=HighNormalVAF,Description="VAF of variant in normal sample is either greater than {0}, or greater than or equal to VAF in tumor sample (as computed by NYGC\'s pileup method)">'.format(MAX_N_VAF))
    header["INFO"].append('##INFO=<ID=MultiCenterCalled,Number=0,Type=Flag,Description="High Confidence variant">')
...
#                    FILTER=uac_filter[varID]
            ...
            if args.called_by in info_dict \
                    and FILTER=="PASS":
                num_centers=len(info_dict[args.called_by].split(','))
                if num_centers>1:
                    INFO=INFO+";MultiCenterCalled"
