...
RSCRIPT_INPUT=${script_root}/../resources/cm/scripts/formatADMIXTUREinputs.R
...
cd $(dirname $INPUT)
numpops=`grep -vw "-" $INPUT.pop | sort | uniq | wc -l`
admixture --supervised $INPUT.bed $numpops

paste $INPUT.pop \
      $INPUT.fam \
      $INPUT.$numpops.Q \
      > $INPUT.admixtureRes.${numpops}pops

# now, read in file to R and remove all non-study sample
# also name the columns appropriately
$R --vanilla --args $INPUT.admixtureRes.${numpops}pops <${RSCRIPT_OUTPUT}
