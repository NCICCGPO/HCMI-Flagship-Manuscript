...
class Variant(Naming):
    ...
    def find_tools(self):
        ...
        for pair in self.pairs:
            ...
            for sample_name in pair:
                found_keys += [key for key in self.uniq_keys if
                               self.not_empty(self.record.samples[sample_name][self.deuniqify_gt(key, sample_name)],
                                              self.deuniqify_gt(key, sample_name))]
            if len(found_keys) > 0:
    ...
    def find_keys(self, record, sample_name):
        ...
        found_keys = [self.uniqify_gt(key, sample_name) for key in format_keys if
                      self.not_empty(record.samples[sample_name][key], key)]
        return found_keys
    ...
    def reduce_samples(self, samples):
        ...
        for sample_name in samples:
            tool = sample_name.split('_')[0]
            tool = tool.split(':')[-1]
            sample_result.append(':'.join([self.replace_empty(
                self.record.samples[sample_name][self.deuniqify_gt(key, sample_name)],
                self.deuniqify_gt(key, sample_name)) for key in uniq_keys if tool == key.split('_')[0]]))
        if sample_result:
