...
def match_cosmic(cosmic_resistance, record, csq_columns):
    ...
    for i, alt in enumerate(record.alts):
        ...
        #  =======================
        kind = 'CosmicCoding'
        ids = csq_dicts[i][kind].split('&')
        legacy_ids = csq_dicts[i][kind +'_LEGACY_ID'].split('&')
        match = cosmic_resistance[cosmic_resistance.GENOMIC_MUTATION_ID.isin(ids)].copy()
        if match.empty:
            match = cosmic_resistance[cosmic_resistance.LEGACY_MUTATION_ID.isin(legacy_ids)].copy()
        # ID Mutation
        if not match.empty:
            for key in ['MUTATION_ID', 'GENOMIC_MUTATION_ID', 'Drug Name', 'Tier']:
