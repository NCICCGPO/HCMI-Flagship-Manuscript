...
def check_formats(vcf_file):
    bcf_in = pysam.VariantFile(vcf_file)
    return bcf_in.header.formats.keys()
...
def __main__():
    ...
    parser.add_argument('-e', '--exome-wgs',
                        dest='exome_wgs',
                        help='Also reuse AD, DP and AF annotation if call was called in both Exome and WGS',
                        action='store_true')
    ...
    # update name of NYGC AD from WGS & Exome runs
    nygc_ad = 'nygc_AD'
    nygc_dp = 'nygc_DP'
    nygc_af = 'nygc_AF'
    if args.exome_wgs:
    ...
    o=open(OUT,"w")
    seen=0
    formats = check_formats(VCF)
    for line in f:
        if not line.startswith("#"):
            line=line.strip()
            toks=line.split("\t")
            format_keys = toks[8].split(':')
        if line.startswith("#"):
...
#                     o.write('##FORMAT=<ID=nygc_AF,Number=A,Type=Float,Description="Allele fraction of alternate allele in the sample computed using a custom pileup-based approach.">\n')
                # add final values
                o.write('##FORMAT=<ID=AD,Number=R,Type=Integer,Description="Allelic depths for the ref and alt alleles computed using a custom pileup-based approach.">\n')
                o.write('##FORMAT=<ID=DP,Number=1,Type=Integer,Description="Depth of coverage: Number of reads covering site computed using a custom pileup-based approach.">\n')
                o.write('##FORMAT=<ID=AF,Number=A,Type=Float,Description="Allele fraction of alternate allele in the sample computed using a custom pileup-based approach.">\n')
                # supporting values
                ...
                seen=1
            o.write(line)
        elif nygc_ad in format_keys and \
                nygc_dp in format_keys and \
                nygc_af in format_keys:
            toks[8] = toks[8] + ":" + ":".join(['AD','DP', 'AF'])
            normal_values = {format_keys[i] : value for i, value in enumerate(toks[9].split(':'))}
            tumor_values = {format_keys[i] : value for i, value in enumerate(toks[10].split(':'))}
            toks[9] = toks[9] + ":" + ":".join([normal_values[nygc_ad],normal_values[nygc_dp], normal_values[nygc_af]])
            toks[10] = toks[10] + ":" + ":".join([tumor_values[nygc_ad],tumor_values[nygc_dp], tumor_values[nygc_af]])
            o.write("\t".join(toks)+"\n")
        elif nygc_ad not in format_keys or \
                nygc_dp not in format_keys or \
                nygc_af not in format_keys:
            new_ids=["AD", "DP", "AF", "nygc_F1R2", "nygc_F2R1", "nygc_FWD", "nygc_REV",
